...
def RunBackscatterDominatedCorkWPairAn(THETA_R,THETA_E,THETA_J,BULK_LORENTZ,INITIAL_RADIUS,COMOVING_DENSITY_COEFF,N_PHOTONS):
    ...
    photon_energies = SD.GetMonoenergeticPhotons(N_PHOTONS,500.0/NATURAL_TO_KEV)
    ...
    for iteration in range(MAX_ITERATIONS):
        ...
        # compton_cross_section_temp = IC.CalcPhotonKNCrossection(photon_energies[idx]*SC.GetLorentzTransform(SC.GetLorentzTransform(photon_wave_vector[idx],emession_frame[idx]),electrons[idx])[:,0])
        # correction_factor_EF = (1.0- cp.sqrt(1.0-1.0/electrons[idx,0]**2)*cp.sum( SC.NormalizeFourVector(SC.GetLorentzTransform(photon_wave_vector[idx],emession_frame[idx]))[:,1:4]*electrons[idx,1:4] ,axis=1) )
        # correction_factor_EM = (1.0- cp.sqrt(1.0-1.0/emession_frame[idx,0]**2)*cp.sum( photon_wave_vector[idx,1:4]*emession_frame[idx,1:4] ,axis=1) )
        # compton_cross_section = compton_cross_section_temp*correction_factor_EF*correction_factor_EM
        ...
        # mean_free_path = cp.clip( GetMeanFreePath(COMOVING_DENSITY_COEFF,emession_frame[idx,0],cp.linalg.norm(photon_position[idx,1:4],axis=1),cp.sum(photon_wave_vector[idx,1:4]*emession_frame[idx,1:4],axis=1),compton_cross_section) , 1.0,1.0e5  )

        # mean_free_path = GetMeanFreePath(COMOVING_DENSITY_COEFF,emession_frame[idx,0],cp.linalg.norm(photon_position[idx,1:4],axis=1),photon_wave_vector[idx,3],compton_cross_section)
        mean_free_path = GetMeanFreePath(COMOVING_DENSITY_COEFF,emession_frame[idx,0],INITIAL_RADIUS*cp.ones(photon_position[idx,0].size,dtype=DTYPE),photon_wave_vector[idx,3],compton_cross_section)
        print("MFP min:",cp.min(mean_free_path))
        print("MFP max:",cp.max(mean_free_path))
        ...
        # print(cp.max(mean_free_path[:,None]))
        ...
        if iteration>=0:
            # out_of_cork = cp.logical_or( (cp.linalg.norm(photon_position[idx,1:4],axis=1) - (INITIAL_RADIUS + photon_position[idx,0]*(cp.sqrt(emession_frame[idx,0]**2-1)/emession_frame[idx,0] ) )) < 1.0e-13,
                                        # cp.arccos(photon_position[idx,3]/cp.linalg.norm(photon_position[idx,1:4],axis=1)) > THETA_J )

            # out_of_cork =  ((cp.linalg.norm(photon_position[idx,1:4],axis=1)-INITIAL_RADIUS) - (photon_position[idx,0]*(cp.sqrt(emession_frame[idx,0]**2-1)/emession_frame[idx,0] ) )) < 1.0e-11
            # out_of_cork = cp.logical_and(SC.GetLorentzTransform(photon_wave_vector[idx], emession_frame[idx])[:,3]<0.0, out_of_cork)
            out_of_cork = distance_rel_cork_inner[idx]<0.0

            # out_of_cork = cp.logical_or( (cp.linalg.norm(photon_position[idx,1:4],axis=1) - (INITIAL_RADIUS + photon_position[idx,0]*cp.sqrt(1.0-1.0/emession_frame[idx,0]**2))) < 1.0e-10,
            #                             cp.arccos(photon_position[idx,3]/cp.linalg.norm(photon_position[idx,1:4],axis=1)) > THETA_J )
            # print(cp.arccos(photon_position[idx,3]/cp.linalg.norm(photon_position[idx,1:4],axis=1))[:20])
            if out_of_cork.any():
                dead_idx = idx[out_of_cork]
                # print("Position",(cp.linalg.norm(photon_position[dead_idx,1:4],axis=1)-INITIAL_RADIUS) )
                # print("Traveled path",(photon_position[dead_idx,0]*(cp.sqrt(emession_frame[dead_idx,0]**2-1)/emession_frame[dead_idx,0] ) ))
        ...
        if iteration > 0:
            photon_polarization_vector[idx] = IC.LorentzTransformPolarizationVector(photon_wave_vector[idx],photon_polarization_vector[idx],emession_frame[idx],FORWARD)
        ...
        # print(cp.max( cp.abs(cp.sum(photon_polarization_vector[idx]*photon_wave_vector[idx], axis=1) )))
