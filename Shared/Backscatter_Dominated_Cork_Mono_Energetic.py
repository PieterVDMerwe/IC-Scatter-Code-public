"""Simulation of backscatter-dominated jet cork with monoenergetic spectrum.
...
Unlike the pair annihilation counterpart, it initializes photons with a single,
discrete energy (monoenergetic).
"""
...
def RunBackscatterDominatedCorkWPairAn(THETA_R, THETA_E, THETA_J, BULK_LORENTZ, INITIAL_RADIUS, COMOVING_DENSITY_COEFF, N_PHOTONS):
    """Runs a Monte Carlo simulation of monoenergetic photons in a jet cork.

    Photons start monoenergetic, propagate, calculate mean free paths, boost
    to comoving/electron frames, scatter, and write out their final Stokes states.

    Args:
        THETA_R: Dimensionless temperature parameter of radiation (not directly used for spectrum here).
