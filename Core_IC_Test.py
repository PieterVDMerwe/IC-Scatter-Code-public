"""Tests and verification routines for Core IC scattering modules.

This script implements various test scenarios including Lorentz boost evaluations,
Klein-Nishina cross-sections, polar and azimuthal scattering angle distributions,
and isotropic / beamed Inverse Compton scattering simulations.
"""
...
def test_lorentz_transform_plot():
    """Tests and plots a Lorentz transformation boost of 4-vectors at rest."""
    N = 10000
...
def test_isotropic_photon_lorentz_transform():
    """Tests Lorentz transforms of an isotropic photon field under a relativistic boost."""
    photon_energies = SD.GetMonoenergeticPhotons(N_PHOTONS, 1.0)
    photon_directions = SD.GetIsotropicDirection(N_PHOTONS)

    N = photon_energies.shape[0]

    # Build 4-vectors of the photons (shape: N x 4)
    momenta = photon_energies[:, None] * photon_directions
    photon_4vectors = cp.concatenate([photon_energies[:, None], momenta], axis=1)

    # Define bulk motion in +z direction with gamma = 10
    gamma_bulk = 10.0

    # Create new_frame array of shape (N,4)
    beta_vec = cp.array([0.0, 0.0, 1.0], dtype=DTYPE)
    beta_vec_tiled = cp.tile(beta_vec, (N, 1))
    gamma_arr = cp.full((N, 1), gamma_bulk)
    new_frame = cp.concatenate([gamma_arr, beta_vec_tiled], axis=1)
...
def RunKNCrossectionTest():
    """Plots the Klein-Nishina cross-section over a range of energy values."""
...
def compton_theta_pdf(mu, epsilon):
    """Calculates the analytic angular distribution PDF for polar Compton scattering.
    ...
    Returns:
        cupy.ndarray: Analytic probability density.
    """
...
def RunScatterMuTest():
    """Verifies polar angle (mu) sampling against analytic Compton distribution."""
    N = 10**8
    E = 10.0
    ...
    # Workaround: local definition of compton_pdf
    theoretical = compton_theta_pdf(bin_centers, E)
...
def run_CalcScPhi_kernel_CuPy(CalcScPhi, xi_phi, photon_energies, mu, scattered_energies):
    """Launches the CalcScPhi kernel to solve for azimuthal angles.

    Args:
        CalcScPhi: Numba CUDA kernel object.
        xi_phi: CuPy array of random numbers.
        photon_energies: CuPy array of photon energies.
        mu: Cosine scattering angles.
        scattered_energies: Scattered energies.

    Returns:
        cupy.ndarray: Sampled phi values.
    """
...
def polarization_pdf(phi, mu, epe, eep):
    """Calculates the analytic azimuthal angle (phi) polarization-dependent PDF.

    Args:
        phi: Trial azimuthal angle.
        mu: Cosine of polar scattering angle.
        epe: scattered energy / initial energy ratio.
        eep: initial energy / scattered energy ratio.

    Returns:
        numpy.ndarray: Analytic probability density.
    """
    y = 1.0 - mu**2
    denom = epe + eep - y
...
def test_phi_sampling_with_cupy_and_kernel():
    """Verifies azimuthal angle (phi) sampling against analytic polarization PDF."""
    N = 10**8
...
def RunIsotropicICScatteringTest():
    """Simulates and plots Inverse Compton scattering spectra for isotropic inputs."""
    fig, ax = plt.subplots()
...
def RunBeamedIsotropicICScatteringTest():
    """Simulates and plots polarization features of beamed Inverse Compton scattering."""
    fig, ax = plt.subplots()
    ...
    Q_binned /= I
    U_binned /= I
    PD = cp.sqrt(Q_binned**2 + U_binned**2)
